...
bool datalink_is_bip(void)
{
    return (Datalink_Transport == DATALINK_BIP);
}

bool datalink_is_mstp(void)
{
    return (Datalink_Transport == DATALINK_MSTP);
}
    ...
    unsigned pdu_len)
{
    ...
    /* Minimal diagnostic: log selected datalink and PDU length (optional) */
#if BACNET_DISCOVERY_DIAGNOSTICS
    ESP_LOGI("datalink", "send via=%s pdu_len=%u bytes=%d dest_mac_len=%u", dl_name, (unsigned)pdu_len, bytes, (unsigned)(dest ? dest->mac_len : 0));
#endif
